...
interface AddNewReviewModalProps {
  onClose: () => void
  productId: number
  onSuccess?: () => void // 👈 Callback cập nhật dữ liệu mượt mà
}

export default function AddNewReviewModal({
  onClose,
  productId,
  onSuccess,
}: AddNewReviewModalProps) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Tải lại danh sách review không cần reload trang
  ...
  return (
    /* CONTAINER MODAL: Giới hạn chiều cao tối đa 90vh, dùng flex-col */
    <div className="relative bg-white rounded-3xl shadow-2xl border border-gray-100 flex flex-col w-full max-w-xl max-h-[90vh] mx-auto overflow-hidden animate-in fade-in zoom-in-95 duration-200">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
        {/* 2. BODY (Có thể cuộn scrollbar tự động khi tràn nội dung) */}
          ...
          <div className="form-group">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div
                ...
                className="flex flex-col items-center justify-center border-2 border-dashed border-emerald-200 bg-emerald-50/30 hover:bg-emerald-50/80 rounded-2xl p-3 cursor-pointer transition-all group min-h-[100px]"
              >
                <CloudUpload
              ...
              <div className="border border-gray-100 rounded-2xl p-2 bg-gray-50/50 flex gap-2 items-center overflow-x-auto min-h-[100px]">
                {previewUrl.length > 0 ? (
